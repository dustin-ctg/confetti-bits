...
class Confetti_Bits_BuddyBoss_Integration extends BP_Integration {
	...
	public function __construct() {
		$this->start(
			'confettibits',
			__( 'Confetti Bits', 'buddyboss-platform-addon' ),
			'confettibits',
			array(
	...
	public function setup_admin_integration_tab() {
		...
		new Confetti_Bits_BuddyBoss_Admin_Integration_Tab(
			...
			$this->name,
			array(
				'root_path'       => CONFETTI_BITS_ADDON_PLUGIN_PATH . '/integration',
				'root_url'        => CONFETTI_BITS_ADDON_PLUGIN_URL . '/integration',
	...
	public function action_links( $links, $file ) {
		...
		if ( CONFETTI_BITS_ADDON_PLUGIN_BASENAME != $file ) {
			return $links;
		}
		...
		return array_merge(
			$links,
			array(
				'settings' => '<a href="' . esc_url( bp_get_admin_url( 'admin.php?page=bp-integrations&tab=bp-confettibits' ) ) . '">' . __( 'Settings', 'buddyboss-platform-addon' ) . '</a>',
			)
		);
	}
}
