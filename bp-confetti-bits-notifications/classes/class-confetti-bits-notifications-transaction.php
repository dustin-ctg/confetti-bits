 ...
 */
class Confetti_Bits_Transaction {
	...
	/**
	 * Log reference of the transaction.
	 *
	 * @var string
	 */
	public $log_ref;
	...
	/**
	 * Set up data related to a specific transaction object.
	 *
	...
	public function populate( $id ) {
		...
		$fetched_transaction = ( ! empty( $bits['transactions'] ) ? current( $bits['transactions'] ) : array() );

		if ( ! empty( $fetched_transaction ) ) {
			$this->id        = (int) $fetched_transaction->id;
			$this->thread_id = (int) $fetched_transaction->thread_id;
			$this->sender_id = (int) $fetched_transaction->sender_id;
			$this->log_ref   = $fetched_transaction->log_ref;
	...
	public function send() {
		...
		$this->log_ref      = apply_filters( 'messages_message_subject_before_save', $this->log_ref, $this->id );
